...
class _GetStartedScreenState extends State<GetStartedScreen> {
  ...
  final List<String> images = [
    ...
    'assets/onboarding/4.png', 'assets/onboarding/5.png',
  ];
  final List<String> captions = ['Know the information', 'Get your equipment', 'Plant a crop', 'Grow with us', 'Increase Yield'];
  ...
  @override
  void dispose() {
    _timer?.cancel(); _pageController.dispose(); emailCtrl.dispose(); passCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> login() async {
    ...
    if (email.isEmpty || password.isEmpty) { _toast('Please enter email and password'); return; }
    ...
    try {
      ...
      if (msg.contains('invalid login credentials')) _toast('Invalid email or password');
      else if (msg.contains('email not confirmed')) _toast('Please verify your email before signing in.');
      else _toast(e.message);
    } catch (e) { _toast('Login failed: $e'); }
                ...
                children: [
                  SizedBox(
                    ...
                    child: PageView.builder(
                      ...
                      itemBuilder: (_, i) => Padding(padding: const EdgeInsets.all(20), child: Image.asset(images[i], fit: BoxFit.contain)),
                    ),
                  ),
                  AnimatedSwitcher(
                    duration: const Duration(milliseconds: 300),
                    child: Text(captions[_currentPage], key: ValueKey(captions[_currentPage]),
                  ...
                  Row(
                    ...
                    children: List.generate(images.length, (i) => AnimatedContainer(
                      ...
                      height: 8, width: _currentPage == i ? 22 : 8,
                      decoration: BoxDecoration(
                        color: Colors.white.withValues(alpha: _currentPage == i ? 1.0 : 0.5),
  ...
  Widget _welcomeUI() {
    return Column(
      ...
      children: [
        ...
        const Text('Create an account or sign in to continue', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey)),
        ...
        const SizedBox(height: 16),
        TextButton(onPressed: () => setState(() => showLogin = true), child: const Text('Already have an account? Sign In', style: TextStyle(color: primaryColor))),
      ],
    );
  }
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(backgroundColor: primaryColor, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30))),
            child: loading ? const CircularProgressIndicator(color: Colors.white) : const Text('Sign In', style: TextStyle(color: Colors.white, fontSize: 16)),
          ),
        ),
        const SizedBox(height: 20),
        TextButton(onPressed: () => setState(() => showLogin = false), child: const Text("Don't have an account? Sign Up", style: TextStyle(color: primaryColor))),
      ],
    );
  }
  ...
  Widget _passwordField() {
    return TextField(
      controller: passCtrl, obscureText: !showPassword,
      decoration: InputDecoration(
