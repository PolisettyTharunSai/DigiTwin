...
class _SignupScreenState extends State<SignupScreen> {
  ...
  void _showVerifyDialog(String email) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => _resendEmail(email), child: const Text('Resend Email', style: TextStyle(color: Colors.black))),
          TextButton(
            onPressed: () {
              Navigator.pop(context);
              Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const GetStartedScreen()), (_) => false);
            },
  ...
  @override
  void dispose() {
    nameCtrl.dispose(); emailCtrl.dispose(); passCtrl.dispose(); confirmPassCtrl.dispose();
    super.dispose();
  }
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: loading ? const CircularProgressIndicator(color: Colors.white) : const Text('Sign Up', style: TextStyle(color: Colors.white, fontSize: 16)),
                ),
              ),
              const SizedBox(height: 20),
              Center(
